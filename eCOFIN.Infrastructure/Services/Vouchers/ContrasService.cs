...
{
    public class ContrasService : IContrasService
    {
        private readonly BilzFinDbContext _context;
        private readonly ILogger<ContrasService> _logger;
        private readonly ILedgerRecalculationService _ledgerRecalculationService;

        public ContrasService(BilzFinDbContext context, ILogger<ContrasService> logger, ILedgerRecalculationService ledgerRecalculationService)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IEnumerable<ExistingContraDto>> GetAllContrasAsync(string accPeriod)
        {
            try
            {
                var q = from cn in _context.CfnContras.AsNoTracking()
                        where cn.CtrlAccperiod == accPeriod
                           && _context.CfnContradetls.AsNoTracking()
                               .Any(d => d.CtrlOnholdno == cn.CtrlOnholdno)
                        orderby cn.VchrNumber descending, cn.VchrDate descending
                        select new ExistingContraDto
                        {
                            CtrlOnHoldNo = cn.CtrlOnholdno,
                            VchrNumber = cn.VchrNumber ?? string.Empty,
                            VchrDate = cn.VchrDate,
                            TotalAmount = cn.VchrTotalamount ?? 0m,
                            BankCode = string.Empty,
                            Description = string.Empty,
                            VchrNarration = cn.VchrNarration ?? string.Empty
                        };
            ...
            {
                _logger.LogError(ex, "Error retrieving Contras.");
                throw new ApplicationException("Error retrieving Contras: " + ex.Message, ex);
            }
        }

        public async Task<ContrasDto?> GetContraByOnHoldNoAsync(string onHoldNo)
        {
            if (string.IsNullOrWhiteSpace(onHoldNo)) return null;
            try
            {
                return await _context.CfnContras.AsNoTracking()
                    .Where(x => x.CtrlOnholdno == onHoldNo)
                    .Select(x => new ContrasDto
                    {
            ...
            {
                _logger.LogError(ex, "Error retrieving Contra header for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Contra header for {onHoldNo}", ex);
            }
        }

        public async Task<List<ContraDetailsDto>> GetContraDetailsByOnHoldNoAsync(string onHoldNo)
        {
            if (string.IsNullOrWhiteSpace(onHoldNo)) return new List<ContraDetailsDto>();
            try
            {
                var details = await _context.CfnContradetls.AsNoTracking()
                    .Where(d => d.CtrlOnholdno == onHoldNo)
                    .OrderBy(d => d.CtrlSequenceno)
                    .Select(d => new ContraDetailsDto
                    {
                        CtrlOnHoldNo = d.CtrlOnholdno,
                        CtrlSequenceNo = (int?)d.CtrlSequenceno,
                        DbCrFlag = d.Dbcrflag,
            ...
            {
                _logger.LogError(ex, "Error retrieving Contra details for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Contra details for {onHoldNo}", ex);
            }
        }
        ...
        public async Task<ContraWithDetailsDto> GetContraWithDetailsAsync(string onHoldNo)
        {
            var result = new ContraWithDetailsDto();
            if (string.IsNullOrWhiteSpace(onHoldNo)) return result;
            try
            {
                var header = await GetContraByOnHoldNoAsync(onHoldNo).ConfigureAwait(false);
                var details = await GetContraDetailsByOnHoldNoAsync(onHoldNo).ConfigureAwait(false);

                result.Header = header ?? new ContrasDto();
                result.Details = details ?? new List<ContraDetailsDto>();
            ...
            {
                _logger.LogError(ex, "Error retrieving Contra with details for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Contra with details for {onHoldNo}", ex);
            }
        }
            ...
            try
            {
                ...
                if (currentNo == null)
                {
                    const string insertSql = @"INSERT INTO CFN_VCHRCONTROL (VOUCHERGROUP, VOUCHERTYPE, ACCPERIOD, STVOUCHERPOSTED, ENVOUCHERPOSTED, STVOUCHERONHOLDSLNUM, ENVOUCHERONHOLDSLNUM, CURRENTONHOLDNO, CURRENTPOSTEDNO, PREFIXTYPE, POSTCOLUMNNAME, CTRL_TRGLOCATIONCODE) 
                        VALUES ('CONT', @voucherType, @accPeriod, 0, 0, 0, 0, 0, 0, LEFT(@voucherType, 3), '', '');";
        ...
        public async Task<string> OnHoldContraAsync(ContrasRequestDto request)
        {
            ...
            try
            {
                ...
                var detailsDto = request.Details ?? new List<ContraDetailsDto>();
                ...
                var header = await _context.CfnContras.FirstOrDefaultAsync(x => x.CtrlOnholdno == onHoldNo) ?? new CfnContra { CtrlOnholdno = onHoldNo, CtrlCreatedon = now };
                ...
                if (_context.Entry(header).State == EntityState.Detached)
                    _context.CfnContras.Add(header);
                ...
                await _context.CfnContradetls.Where(x => x.CtrlOnholdno == onHoldNo).ExecuteDeleteAsync();
                ...
                var contraDetails = detailsDto.Select(d =>
                {
                ...
                if (contraDetails.Any())
                    await _context.CfnContradetls.AddRangeAsync(contraDetails);

                header.VchrTotalamount = contraDetails.Where(x => string.Equals(x.Dbcrflag, "C", StringComparison.OrdinalIgnoreCase)).Sum(x => x.Dbcramount);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error saving Contra (OnHold)");
                throw new ApplicationException("Error saving Contra (OnHold): " + ex.Message, ex);
            }
        }

        public async Task<string> PostContraAsync(ContrasRequestDto request)
        {
            ArgumentNullException.ThrowIfNull(request?.VoucherData);
            await using var tx = await _context.Database.BeginTransactionAsync(IsolationLevel.ReadCommitted);
            try
            ...
            {
                await tx.RollbackAsync();
                _logger.LogError(ex, "Error saving Contra (Post)");
                throw new ApplicationException("Error saving Contra (Post): " + ex.Message, ex);
            }
        }
        ...
        private void DetachVoucherEntries(string onHoldNo)
        {
            var stale = _context.ChangeTracker.Entries()
                ...
                {
                    CfnContradetl x => x.CtrlOnholdno == onHoldNo,
