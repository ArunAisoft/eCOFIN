...
{
    public class CashReceiptsService : ICashReceiptsService
    {
        private readonly BilzFinDbContext _context;
        private readonly ILogger<CashReceiptsService> _logger;
        private readonly ILedgerRecalculationService _ledgerRecalculationService;

        public CashReceiptsService(BilzFinDbContext context, ILogger<CashReceiptsService> logger, ILedgerRecalculationService ledgerRecalculationService)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IEnumerable<ExistingCashReceiptDto>> GetAllCashReceiptsAsync(string accPeriod)
        {
            try
            {
                var q = from cr in _context.CfnCashreceipts.AsNoTracking()
                        join d in _context.CfnCshrdetails.AsNoTracking()
                            on cr.CtrlOnholdno equals d.CtrlOnholdno
                        join acc in _context.CfnAccounts.AsNoTracking()
                            on cr.CashAccount equals acc.Accountcode
                        where d.Dbcrflag == "D"
                           && cr.CtrlAccperiod == accPeriod
                        group new { cr, d, acc } by new
                        {
                            cr.CtrlOnholdno,
                            cr.VchrNumber,
                            cr.VchrDate,
                        ...
                        select new ExistingCashReceiptDto
                        {
                            ...
                            CashAccount = g.Key.CashAccount ?? string.Empty,
            ...
            {
                _logger.LogError(ex, "Error retrieving Cash Receipts.");
                throw new ApplicationException("Error retrieving Cash Receipts: " + ex.Message, ex);
            }
        }

        public async Task<CashReceiptsDto?> GetCashReceiptByOnHoldNoAsync(string onHoldNo)
        {
            if (string.IsNullOrWhiteSpace(onHoldNo)) return null;
            try
            {
                return await _context.CfnCashreceipts.AsNoTracking()
                    .Where(x => x.CtrlOnholdno == onHoldNo)
                    .Select(x => new CashReceiptsDto
                    {
            ...
            {
                _logger.LogError(ex, "Error retrieving Cash Receipt header for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Cash Receipt header for {onHoldNo}", ex);
            }
        }

        public async Task<List<CashrDetailsDto>> GetCashrDetailsByOnHoldNoAsync(string onHoldNo)
        {
            if (string.IsNullOrWhiteSpace(onHoldNo)) return new List<CashrDetailsDto>();
            try
            {
                var details = await _context.CfnCshrdetails.AsNoTracking()
                    .Where(d => d.CtrlOnholdno == onHoldNo)
                    .OrderBy(d => d.CtrlSequenceno)
                    .Select(d => new CashrDetailsDto
                    {
            ...
            {
                _logger.LogError(ex, "Error retrieving Cash Receipt details for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Cash Receipt details for {onHoldNo}", ex);
            }
        }
        ...
        public async Task<CashReceiptWithDetailsDto> GetCashReceiptWithDetailsAsync(string onHoldNo)
        {
            var result = new CashReceiptWithDetailsDto();
            if (string.IsNullOrWhiteSpace(onHoldNo)) return result;
            try
            {
                var header = await GetCashReceiptByOnHoldNoAsync(onHoldNo).ConfigureAwait(false);
                var details = await GetCashrDetailsByOnHoldNoAsync(onHoldNo).ConfigureAwait(false);

                result.Header = header ?? new CashReceiptsDto();
                result.Details = details ?? new List<CashrDetailsDto>();
            ...
            {
                _logger.LogError(ex, "Error retrieving Cash Receipt with details for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Cash Receipt with details for {onHoldNo}", ex);
            }
        }
            ...
            try
            {
                ...
                if (currentNo == null)
                {
                    const string insertSql = @"INSERT INTO CFN_VCHRCONTROL (VOUCHERGROUP, VOUCHERTYPE, ACCPERIOD, STVOUCHERPOSTED, ENVOUCHERPOSTED, STVOUCHERONHOLDSLNUM, ENVOUCHERONHOLDSLNUM, CURRENTONHOLDNO, CURRENTPOSTEDNO, PREFIXTYPE, POSTCOLUMNNAME, CTRL_TRGLOCATIONCODE) 
                        VALUES ('CASR', @voucherType, @accPeriod, 0, 0, 0, 0, 0, 0, LEFT(@voucherType, 3), '', '');";
        ...
        public async Task<string> OnHoldCashReceiptAsync(CashReceiptsRequestDto request)
        {
            ...
            try
            {
                ...
                var detailsDto = request.Details ?? new List<CashrDetailsDto>();
                ...
                var header = await _context.CfnCashreceipts.FirstOrDefaultAsync(x => x.CtrlOnholdno == onHoldNo) ?? new CfnCashreceipt { CtrlOnholdno = onHoldNo, CtrlCreatedon = now };
                ...
                if (_context.Entry(header).State == EntityState.Detached)
                    _context.CfnCashreceipts.Add(header);
                ...
                await _context.CfnCshrdetails.Where(x => x.CtrlOnholdno == onHoldNo).ExecuteDeleteAsync();
                ...
                var cshrDetails = detailsDto.Select(d =>
                {
                ...
                if (cshrDetails.Any())
                    await _context.CfnCshrdetails.AddRangeAsync(cshrDetails);
                ...
                header.VchrTotalamount = cshrDetails.Where(x => string.Equals(x.Dbcrflag, "C", StringComparison.OrdinalIgnoreCase)).Sum(x => x.Drcramount);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error saving Cash Receipt (OnHold)");
                throw new ApplicationException("Error saving Cash Receipt (OnHold): " + ex.Message, ex);
            }
        }

        public async Task<string> PostCashReceiptAsync(CashReceiptsRequestDto request)
        {
            ArgumentNullException.ThrowIfNull(request?.VoucherData);
            await using var tx = await _context.Database.BeginTransactionAsync(IsolationLevel.ReadCommitted);
            try
            {
                ...
                var cshrDetails = detailsDto.Select(d => new CfnCshrdetail
                {
                    CtrlOnholdno = onHoldNo,
                    CtrlSequenceno = d.CtrlSequenceNo!.Value,
                    Dbcrflag = string.IsNullOrWhiteSpace(d.DbCrFlag) ? "C" : d.DbCrFlag,
                    Accountcode = d.AccountCode ?? string.Empty,
                    Subaccountcode = d.SubAccountCode,
                    Referencenumber = d.InstrumentNo,
                    Referencedate = d.InstrumentDate,
                    Drcramount = d.DrCrAmount ?? 0m,
            ...
            {
                await tx.RollbackAsync();
                _logger.LogError(ex, "Error saving Cash Receipt (Post)");
                throw new ApplicationException("Error saving Cash Receipt (Post): " + ex.Message, ex);
            }
        }
        ...
        private void DetachVoucherEntries(string onHoldNo)
        {
            var stale = _context.ChangeTracker.Entries()
                ...
                {
                    CfnCshrdetail x => x.CtrlOnholdno == onHoldNo,
