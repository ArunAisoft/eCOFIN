...
namespace eCOFIN.Infrastructure.Services.Vouchers
{
    public class CreditNotesService : ICreditNotesService
    {
        ...
        private readonly ILogger<CreditNotesService> _logger;
        private readonly ILedgerRecalculationService _ledgerRecalculationService;

        public CreditNotesService(BilzFinDbContext context, ILogger<CreditNotesService> logger, ILedgerRecalculationService ledgerRecalculationService)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IEnumerable<ExistingCreditNoteDto>> GetAllCreditNotesAsync(string accPeriod)
        {
            try
            {
                var q = from cn in _context.CfnCreditnotes.AsNoTracking()
                        join d in _context.CfnCrdndetails.AsNoTracking()
                        ...
                        select new ExistingCreditNoteDto
                        {
            ...
            catch (Exception ex)
            {
                ...
                throw new ApplicationException("Error retrieving Credit Notes: " + ex.Message, ex);
            }
        }

        public async Task<CreditNotesDto?> GetCreditNoteByOnHoldNoAsync(string onHoldNo)
        {
            if (string.IsNullOrWhiteSpace(onHoldNo)) return null;
            try
            {
                return await _context.CfnCreditnotes.AsNoTracking()
                    .Where(x => x.CtrlOnholdno == onHoldNo)
                    .Select(x => new CreditNotesDto
                    {
            ...
            {
                _logger.LogError(ex, "Error retrieving Credit Note header for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Credit Note header for {onHoldNo}", ex);
            }
        }

        public async Task<List<CrdnDetailsDto>> GetCrdnDetailsByOnHoldNoAsync(string onHoldNo)
        {
            if (string.IsNullOrWhiteSpace(onHoldNo)) return new List<CrdnDetailsDto>();
            try
            {
                var details = await _context.CfnCrdndetails.AsNoTracking()
                    .Where(d => d.CtrlOnholdno == onHoldNo)
                    .OrderBy(d => d.CtrlSequenceno)
                    .Select(d => new CrdnDetailsDto
                    {
            ...
            {
                _logger.LogError(ex, "Error retrieving Credit Note details for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Credit Note details for {onHoldNo}", ex);
            }
        }
        ...
        public async Task<CreditNoteWithDetailsDto> GetCreditNoteWithDetailsAsync(string onHoldNo)
        {
            var result = new CreditNoteWithDetailsDto();
            if (string.IsNullOrWhiteSpace(onHoldNo)) return result;
            try
            {
                var header = await GetCreditNoteByOnHoldNoAsync(onHoldNo).ConfigureAwait(false);
                var details = await GetCrdnDetailsByOnHoldNoAsync(onHoldNo).ConfigureAwait(false);

                result.Header = header ?? new CreditNotesDto();
                result.Details = details ?? new List<CrdnDetailsDto>();

                var totalCredit = result.Details.Where(d => string.Equals(d.DbCrFlag, "D", StringComparison.OrdinalIgnoreCase)).Sum(d => d.DrCrAmount ?? 0m);

                result.Header.Balance ??= totalCredit;
                return result;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving Credit Note with details for {OnHoldNo}", onHoldNo);
                throw new ApplicationException($"Error retrieving Credit Note with details for {onHoldNo}", ex);
            }
        }
            ...
            try
            {
                ...
                if (currentNo == null)
                {
                    const string insertSql = @"INSERT INTO CFN_VCHRCONTROL (VOUCHERGROUP, VOUCHERTYPE, ACCPERIOD, STVOUCHERPOSTED, ENVOUCHERPOSTED, STVOUCHERONHOLDSLNUM, ENVOUCHERONHOLDSLNUM, CURRENTONHOLDNO, CURRENTPOSTEDNO, PREFIXTYPE, POSTCOLUMNNAME, CTRL_TRGLOCATIONCODE) 
                        VALUES ('CRDT', @voucherType, @accPeriod, 0, 0, 0, 0, 0, 0, LEFT(@voucherType, 3), '', '');";
        ...
        public async Task<string> OnHoldCreditNoteAsync(CreditNotesRequestDto request)
        {
            ...
            try
            {
                ...
                var detailsDto = request.Details ?? new List<CrdnDetailsDto>();
                ...
                var header = await _context.CfnCreditnotes.FirstOrDefaultAsync(x => x.CtrlOnholdno == onHoldNo) ?? new CfnCreditnote { CtrlOnholdno = onHoldNo, CtrlCreatedon = now };
                ...
                if (_context.Entry(header).State == EntityState.Detached)
                    _context.CfnCreditnotes.Add(header);
                ...
                await _context.CfnCrdndetails.Where(x => x.CtrlOnholdno == onHoldNo).ExecuteDeleteAsync();
                ...
                var crdnDetails = detailsDto.Select(d =>
                {
                ...
                if (crdnDetails.Any())
                    await _context.CfnCrdndetails.AddRangeAsync(crdnDetails);
                ...
                header.VchrTotalamount = crdnDetails.Where(x => string.Equals(x.Dbcrflag, "D", StringComparison.OrdinalIgnoreCase)).Sum(x => x.Dbcramount);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error saving Credit Note (OnHold)");
                throw new ApplicationException("Error saving Credit Note (OnHold): " + ex.Message, ex);
            }
        }

        public async Task<string> PostCreditNoteAsync(CreditNotesRequestDto request)
        {
            ArgumentNullException.ThrowIfNull(request?.VoucherData);
            await using var tx = await _context.Database.BeginTransactionAsync(IsolationLevel.ReadCommitted);
            try
            ...
            {
                await tx.RollbackAsync();
                _logger.LogError(ex, "Error saving Credit Note (Post)");
                throw new ApplicationException("Error saving Credit Note (Post): " + ex.Message, ex);
            }
        }
        ...
        private void DetachVoucherEntries(string onHoldNo)
        {
            var stale = _context.ChangeTracker.Entries()
                ...
                {
                    CfnCrdndetail x => x.CtrlOnholdno == onHoldNo,
