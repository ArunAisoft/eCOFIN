...
namespace eCOFIN.API.Controllers.Vouchers
{
    ...
    public class CreditNotesController : ControllerBase
    {
        ...
        public CreditNotesController(ICreditNotesService creditnotesService, ILogger<CreditNotesController> logger)
        {
            _creditnotesService = creditnotesService;
            _logger = logger;
        }

        [HttpGet("GetAllCreditNotes")]
        public async Task<IActionResult> GetAllCreditNotes(string accPeriod)
        {
            try
            {
                ...
                var vouchers = await _creditnotesService.GetAllCreditNotesAsync(normalized);
        ...
        [HttpGet("GetCreditNoteWithDetails")]
        public async Task<IActionResult> GetCreditNoteWithDetails(string onHoldNo)
        {
            try
            {
                ...
                var voucher = await _creditnotesService.GetCreditNoteWithDetailsAsync(onHoldNo);
                if (voucher == null)
        ...
        [HttpPost("OnHoldCreditNote")]
        public async Task<IActionResult> OnHoldCreditNote([FromBody] CreditNotesRequestDto request)
        {
            try
            {
                ...
                var onHoldNo = await _creditnotesService.OnHoldCreditNoteAsync(request).ConfigureAwait(false);
        ...
        [HttpPost("PostCreditNote")]
        public async Task<IActionResult> PostCreditNote([FromBody] CreditNotesRequestDto request)
        {
            try
            {
                if (request == null || request.VoucherData == null)
                ...
                var voucherNumber = await _creditnotesService.PostCreditNoteAsync(request).ConfigureAwait(false);
